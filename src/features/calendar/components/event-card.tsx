...
export function EventCard({
  ...
  const handleClick = () => {
    // Only handle click for exam events
    if (event.type === 'exam' && event.examId) {
      if (event.isTeacherExam) {
        // Teacher: go to edit page
        navigate({
          to: '/exam/edit/$id',
          params: { id: event.examId.toString() },
        })
      } else {
        // Student: go to take exam page
        navigate({
          to: '/exam/$examId/take',
          params: { examId: event.examId.toString() },
        })
      }
    }
  }
